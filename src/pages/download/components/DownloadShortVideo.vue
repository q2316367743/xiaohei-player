  ...
  <div class="p-8px">
    <t-card>
      ...
      <t-form-item label="平台" label-align="top">
        <t-select v-model="platform" :options="platformOptions" placeholder="请选择平台"/>
      </t-form-item>
      <t-form-item v-if="platform !== 'auto'" label="插件" label-align="top">
        <t-select v-model="plugin" :options="pluginOptions" placeholder="请选择插件"/>
      </t-form-item>
...
import {
  ...
  DownloadPluginPlatformOptions,
  type DownloadPluginView
} from "@/entity/main/DownloadPlugin.ts";
import {LocalName} from "@/global/LocalName.ts";
import {type DownloadResult, pluginAutoParse, pluginParse} from "@/module/download";
...
const url = ref('');
const platform = useLocalStorage<DownloadPluginPlatform | 'auto'>(LocalName.PAGE_DOWNLOAD_PLATFORM, 'auto');
...
const platformOptions: Array<CommonOption> = [
  {
    label: '自动',
    value: 'auto'
  },
...
const pluginOptions = computed(() => plugins.value.filter(e => e.platform.includes(platform.value as any)).map(e => ({
  label: e.name,
  value: e.id
})))
...
const parse = async () => {
  ...
  if (platform.value === 'auto') {
    result.value = await pluginAutoParse(url.value);
  }else {
...
@import "less/DownloadShortVideo.less";
</style>
